...
import cc.dreamcode.antylogout.libs.cc.dreamcode.command.CommandPathMeta;
...
public class InvalidUsageHandlerImpl implements InvalidUsageHandler
{
    ...
    public void handle(@NonNull final DreamSender<?> dreamSender, @NonNull final Optional<CommandMeta> optionalCommandMeta, @NonNull final CommandInput commandInput) {
        ...
        if (commandPathMetas.isEmpty()) {
            this.messageConfig.pathNotFound.send(bukkitSender.getHandler(), (Map<String, Object>)new MapBuilder<String, String>().put("label", "/" + commandMeta.getCommandEntry().getName()).put("description", commandMeta.getCommandEntry().getDescription()).build());
            return;
        }
        this.messageConfig.usage.send(bukkitSender.getHandler(), (Map<String, Object>)new MapBuilder<String, String>().put("label", "/" + commandMeta.getCommandEntry().getName()).put("description", commandMeta.getCommandEntry().getDescription()).build());
