...
final class KeyImpl implements Key
{
    ...
    private static void checkError(final String name, final String checkPart, final String namespace, final String value, final OptionalInt index, final String pattern) {
        if (index.isPresent()) {
            ...
            throw new InvalidKeyException(namespace, value, String.format("Non " + pattern + " character in %s of Key[%s] at index %d ('%s', bytes: %s)", new Object[] { name, asString(namespace, value), indexValue, character, Arrays.toString(String.valueOf(character).getBytes(StandardCharsets.UTF_8)) }));
        }
    }
