...
import cc.dreamcode.antylogout.libs.net.kyori.adventure.text.renderer.ComponentRenderer;
...
final class TextReplacementRenderer implements ComponentRenderer<State>
{
    ...
    public Component render(@NotNull final Component component, @NotNull final State state) {
        ...
        if (state.running) {
            ...
            boolean first = true;
            for (int j = 0; j < oldChildrenSize; ++j) {
                final Component child2 = (Component)oldChildren.get(j);
                ...
                if (replaced2 != child2) {
                    ...
                    if (first) {
                        children.addAll((Collection)oldChildren.subList(0, j));
                    }
                    first = false;
                }
                if (children != null) {
    ...
    static final class State
    {
        ...
        State(@NotNull final Pattern pattern, @NotNull final BiFunction<MatchResult, TextComponent.Builder, ComponentLike> replacement, final TextReplacementConfig.Condition continuer, final boolean replaceInsideHoverEvents) {
            this.running = true;
            this.matchCount = 0;
            this.replaceCount = 0;
