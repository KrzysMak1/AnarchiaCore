...
import cc.dreamcode.antylogout.libs.com.google.gson.InstanceCreator;
...
public final class ConstructorConstructor
{
    ...
    public <T> ObjectConstructor<T> get(final TypeToken<T> typeToken) {
        ...
        final InstanceCreator<T> typeCreator = (InstanceCreator<T>)this.instanceCreators.get((Object)type);
        if (typeCreator != null) {
        ...
        final InstanceCreator<T> rawTypeCreator = (InstanceCreator<T>)this.instanceCreators.get((Object)rawType);
        ...
        final String message = "Unable to create instance of " + (Object)rawType + "; ReflectionAccessFilter does not permit using reflection or Unsafe. Register an InstanceCreator or a TypeAdapter for this type or adjust the access filter to allow using reflection.";
        return new ObjectConstructor<T>() {
            @Override
            public T construct() {
            ...
            return new ObjectConstructor<T>() {
                ...
                public T construct() {
                    ...
                    if (elementType instanceof Class) {
                        final T set = (T)EnumSet.noneOf((Class)elementType);
                        return set;
                    }
                    throw new JsonIOException("Invalid EnumSet type: " + type.toString());
                }
        ...
        if (rawType == EnumMap.class) {
            return new ObjectConstructor<T>() {
                ...
                public T construct() {
                    ...
                    final Type elementType = ((ParameterizedType)type).getActualTypeArguments()[0];
                    if (elementType instanceof Class) {
                        final T map = (T)new EnumMap((Class)elementType);
                        return map;
                    }
                    throw new JsonIOException("Invalid EnumMap type: " + type.toString());
                }
    ...
    private static <T> ObjectConstructor<T> newDefaultConstructor(final Class<? super T> rawType, final ReflectionAccessFilter.FilterResult filterResult) {
        ...
        final boolean canAccess = filterResult == ReflectionAccessFilter.FilterResult.ALLOW || (ReflectionAccessFilterHelper.canAccess((AccessibleObject)constructor, null) && (filterResult != ReflectionAccessFilter.FilterResult.BLOCK_ALL || Modifier.isPublic(constructor.getModifiers())));
        if (!canAccess) {
            final String message = "Unable to invoke no-args constructor of " + (Object)rawType + "; constructor is not accessible and ReflectionAccessFilter does not permit making it accessible. Register an InstanceCreator or a TypeAdapter for this type, change the visibility of the constructor or adjust the access filter.";
            return new ObjectConstructor<T>() {
                @Override
                public T construct() {
                    throw new JsonIOException(message);
        ...
        return new ObjectConstructor<T>() {
            ...
            public T construct() {
                try {
                    final T newInstance = (T)constructor.newInstance(new Object[0]);
                    return newInstance;
                }
                catch (final InstantiationException e) {
                    throw new RuntimeException("Failed to invoke " + (Object)constructor + " with no args", (Throwable)e);
                }
            ...
            return new ObjectConstructor<T>() {
                ...
                public T construct() {
                    try {
                        ...
                        throw new RuntimeException("Unable to create instance of " + (Object)rawType + ". Registering an InstanceCreator or a TypeAdapter for this type, or adding a no-args constructor may fix this problem.", (Throwable)e);
                    }
                }
            };
        }
